...
func Record(req ListRequest) (ListResponse, error) {
	apiReq := request.New(req.AppKey, req.AppSecret)
	vals, err := apiReq.Post(request.URLRoot+"/yonbip/sd/pricing/record", req.ToValues())
	if err != nil {
	...
	if len(res.Data.RecordList) == 0 {
		return res, fmt.Errorf("%w error: not found sale-order list", request.ErrYonSuiteAPIBizError)
	}
