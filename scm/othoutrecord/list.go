...
func List(req ListRequest) (ListResponse, error) {
	...
	vals, err := apiReq.Post(request.URLRoot+"/yonbip/scm/othoutrecord/list", req.ToValues())
	if err != nil {
	...
	if len(res.Data.RecordList) == 0 {
		return *res, fmt.Errorf("%w error: not found other-out list", request.ErrYonSuiteAPIBizError)
	}
