...
type SingleSaveResponse struct {
	...
	// 	SucessCount string           `json:"sucessCount"`
	// 	FailCount   string           `json:"failCount"`
	// 	Messages    []string         `json:"messages"`
	// 	Infos       []request.Values `json:"infos"`
	// 	FailInfos   []request.Values `json:"failInfos"`
	// } `json:"data"`
}

func Save(req SingleSaveRequest) (SingleSaveResponse, error) {
	apiReq := request.New(req.AppKey, req.AppSecret)
	vals, err := apiReq.Post(request.NewURLRoot+"/scm/othoutrecord/single/save", req.ToValues())
	if err != nil {
